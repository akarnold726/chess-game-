...
Game::Game(sf::Color c1, sf::Color c2)
{
    ...
    infoResstart.setOutlineThickness(-5.f);
    infoRestart.setOutlineColor(sf::Color::Black);
    infoRestart.setPosition(sf::Vector2f(180.f, 60.f));
...
void Game::Start(sf::Color c1, sf::Color c2)
{
    number_of_moves = 0;
    isOver = false;
    gameStatus = true, whiteTurn = 1, selecter = false;
    selected_piece = NULL;
    for(int i = 0; i < 8; i++)
    {
        b_pawn[i]->y = i;
        ...
        w_pawn[i]->piece.seetScale(sf::Vector2f(0.375f, 0.375f));  
    }
    b_bishop[0]->y = 2;
    b_bishop[1]->y = 5;
    b_rook[0]-> y = 0;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        b_bishop[i]->piece.setOrigin(100.of * b_bishop[i]->x, 100.0f * b_bishop[i]->y);
...
void Game::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
    ...
    {
        for(int i = 0; i < newmmoves.size(); i++)
        {
    ...
    if (valid)
    {
        ...
        if (Cells[x][y].occupied_color !=0 && Cells[x][y].occupied_color !=Cells[a][b].occupied_color)
        {
            ...
            else
            {
                for (int i = 0; i < blackPieces.size(); i++)
                {
                    if (blackPiece[i]->x == x && blackPiece[i]->y == y)
                    {
